...
async function items(parent, args, { prisma }) {
  ...
  if (args.containsid)
    itemsRes = itemsRes.filter((value) =>
      args.containsid.includes(value.itemId)
    );
  return itemsRes;
}
  ...
  try {
    ...
    if (filter.length > 0) {
      response = response.filter((r) =>
        filter.reduce((acc, filterToken) => {
          if (Array.isArray(r[filterToken.matchPath])) {
            return (
              acc ||
              r[filterToken.matchPath].reduce(
                (acc, cat) =>
                  acc ||
                  cat.toLowerCase().includes(filterToken.match.toLowerCase()),
                false
              )
            );
          } else {
...
function sortTrans(transactions, sortBy, sort, groupBy) {
  ...
  switch (groupBy) {
    ...
    case Groups.DAY:
      transactions.sort(
        getSortfn((a, b) => new Date(a.date) - new Date(b.date))
      );
      break;
    case Groups.WEEK:
      transactions.sort(
        getSortfn((a, b) =>
          new moment(a.groupid, "YYYY [week] ww").diff(
            new moment(b.groupid, "YYYY [week] ww")
          )
        )
      );
      break;
...
function groupBy(transactions, groupid) {
  if (groupid === "TRANSACTION") return transactions;
