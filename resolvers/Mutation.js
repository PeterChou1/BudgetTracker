...
async function signup(parent, args, {res, req, prisma}) {
    ...
    try {
        ...
        const user = await prisma.user.create({ data: { username: args.username, plaidAcc: '[]', password: password}});
        let minLen = 7;
        let format = /[ `!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/;
        if (args.password.length < minLen) throw new Error("Password too short, needs at least "+minLen+" characters");
        else if (!format.test(args.password)) throw new Error("Password needs at least 1 special character");
        else if (!/\d/.test(args.password)) throw new Error("Password needs at least 1 number");
